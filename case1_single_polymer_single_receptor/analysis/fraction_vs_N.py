...
============================
Post-processes EXISTING outputs from a custom LAMMPS cluster-analysis
algorithm. Does NOT rerun clustering. Automatically:

  - walks every ROOT_DIRS entry
  - discovers every N_* topology folder inside it
  - reads the already-computed .dat time-series files
      A_nclusters.dat, B_nclusters.dat, receptor_nclusters.dat
      A_mean_cluster_size.dat, B_mean_cluster_size.dat, receptor_mean_cluster_size.dat
      A_largest_cluster_rg.dat, B_largest_cluster_rg.dat, receptor_largest_cluster_rg.dat
      A_largest_cluster_fraction.dat, B_largest_cluster_fraction.dat,
      receptor_largest_cluster_fraction.dat
  - reads lammps.data once per topology to get per-type atom counts
    (needed to recombine A+B fractions into a single "polymer" fraction)
  - averages every quantity over frames >= EQUILIBRATION_FRAME
  - writes CSV summaries and publication-quality comparison figures

Edit the CONFIGURATION block below, then run:
    python cluster_comparison_analysis.py
"""
...
ROOT_LABELS = {
    ROOT_DIRS[0]: "ε = 0.5",
    ROOT_DIRS[1]: "No Interaction",
}
EQUILIBRATION_FRAME = 0          # frames >= this value are averaged
OUTPUT_DIR = Path("cluster_comparison_output")

# LAMMPS atom types -> species (edit if your typing differs)
POLYMER_ATOM_TYPES = (1, 2)      # type1 = Polymer A, type2 = Polymer B
RECEPTOR_ATOM_TYPES = (3, 4)     # type3 = Receptor C, type4 = Receptor D

# species tags used in the .dat filenames written by the clustering code.
# "A"/"B" are the two polymer types, "receptor" is the combined C+D file
# that already exists per your file listing. "C"/"D" are included so the
# script will automatically pick them up too IF standalone C_*.dat /
# D_*.dat files exist, or if a per-type breakdown can be found inside
# receptor_receptor_details.dat (see try_parse_receptor_details below).
SPECIES_TAGS = ["A", "B", "C", "D", "receptor"]
RECEPTOR_DETAIL_SPECIES = ["C", "D"]   # only these fall back to the details file
...
class TopologyResult:
    ...
    topology: str
    N: int
    # quantity -> species -> (mean, std, n_frames)
    stats: dict = field(default_factory=dict)
    polymer_fraction: tuple = None     # (mean, std, n_frames) or None
...
def discover_topology_folders(root_dir: Path):
    """Find every N_* folder inside root_dir, return sorted (N, path) list."""
...
def find_dat_file(topology_dir: Path, species_tag: str, quantity: str):
    """Locate the .dat file for a given species/quantity, tolerant of
    minor naming variation (case, underscores). The species tag itself
    must appear as an exact token in the candidate filename -- this
    prevents e.g. a lookup for species 'C' from being fuzzy-matched onto
    'A_largest_cluster_fraction.dat' just because most of the other
    tokens (largest/cluster/fraction) overlap."""
    expected = f"{species_tag}_{DAT_SUFFIX[quantity]}"
    candidate = topology_dir / expected
    if candidate.exists():
        return candidate
    # fallback: fuzzy search, but require the species tag as an exact token
...
# =====================================================================
# 4. FILE PARSING (auto-detects header / column names)
# =====================================================================

def read_dat_auto(path: Path) -> pd.DataFrame:
    """Read a .dat file whose exact column layout is not guaranteed.
    Tries, in order: whitespace-delimited with a '#' header line,
    whitespace-delimited with no header, then comma-delimited."""
...
def detect_column(df: pd.DataFrame, hints, fallback_index=None):
    """Return the column name best matching a list of hint substrings."""
...
def compute_stats(df: pd.DataFrame, value_col: str, frame_col: str,
                   equil_frame: int):
    """Mean/std/n over frames >= equil_frame, using NumPy."""
    sub = df[df[frame_col] >= equil_frame]
...
def try_parse_receptor_details(topology_dir: Path, species_tag: str,
    ...
    """Best-effort fallback: if standalone C_*.dat / D_*.dat files don't
    exist, look inside receptor_receptor_details.dat for a per-type
    breakdown column (e.g. a column named like 'C_fraction' or
    'type3_nclusters'). Returns None (silently) if the file doesn't
    exist or no matching column can be identified."""
...
def load_quantity(topology_dir: Path, species_tag: str, quantity: str,
                   equil_frame: int):
    """Load one .dat file and return (mean, std, n) or None. For C/D
    species, falls back to parsing receptor_receptor_details.dat if no
    standalone file is found."""
...
# =====================================================================
# 5. lammps.data ATOM-TYPE COUNTS (for recombining A+B -> polymer)
# =====================================================================

def parse_lammps_type_counts(lammps_data_path: Path):
    """Return {atom_type_int: count} by parsing the Atoms section of a
    LAMMPS data file. Tolerant of atom_style (atomic/full/molecular)."""
    text = lammps_data_path.read_text(errors="ignore").splitlines()

    # number of atom types, from header line "N atom types"
    num_types = None
    for line in text:
    ...
    # candidate type-column indices: try col 1 (atomic: id type ...)
    # then col 2 (full/molecular: id mol type ...). A column only
    # qualifies as the type column if its values are integers within
    # [1, num_types] AND it actually varies (rules out a constant
    # molecule-id column of all 1s being mistaken for the type column).
...
def compute_combined_fractions(topology_dir: Path, equil_frame: int):
    """Recombine A + B fraction time series (weighted by true atom counts
    from lammps.data) into a single polymer fraction. Receptor fraction
    is read directly since receptor_*.dat already represents type3+type4."""
    ...
    # per-frame merge of A and B fraction files, weighted by atom counts
    ...
    if a_series is not None and b_series is not None:
        ...
        # only one polymer species has data; use it directly (fraction of
        # that species already; not exact but best available signal)
...
def process_all():
    results = []  # list of TopologyResult
        ...
        for N, topo_dir in topo_list:
            ...
            for quantity in ["nclusters", "mean_cluster_size",
                ...
                for species_tag in SPECIES_TAGS:
                    stat = load_quantity(topo_dir, species_tag, quantity,
                                          EQUILIBRATION_FRAME)
                    res.stats[quantity][species_tag] = stat

            # combined (atom-count-weighted) polymer & receptor fractions,
            # in addition to the individual-species fractions above
            polymer_stats, receptor_stats = compute_combined_fractions(
                topo_dir, EQUILIBRATION_FRAME)
...
def plot_per_species_figures(quantity_df, ylabel, title_prefix, quantity_key):
    """Given a long-format dataframe with a Species column (as produced by
    write_quantity_csv), emit one SEPARATE figure per species, each with
    one line per root dir. Filenames: {species}_{quantity_key}_vs_N.png"""
...
def main():
    ...
    print("\nGenerating figures ...")
    # combined (atom-count-weighted) polymer & receptor fraction figures
    ...
    # multi-species overlay figures (kept for a compact side-by-side view)
    ...
    # one SEPARATE figure per atom type (A, B, C, D, receptor) for each
